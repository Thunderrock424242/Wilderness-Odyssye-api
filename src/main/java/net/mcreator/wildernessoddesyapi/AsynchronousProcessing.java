...
public class AsynchronousProcessing {
	...
	public static void runAsyncTask(Runnable task) {
		CompletableFuture.runAsync(task, executor).thenAccept(result -> LOGGER.info("Async task completed")).exceptionally(throwable -> {
	...
	public static <T> CompletableFuture<T> supplyAsyncTask(java.util.function.Supplier<T> task) {
		return CompletableFuture.supplyAsync(task, executor).exceptionally(throwable -> {
			LOGGER.error("Async task failed", throwable);
			return null;
		});
	}
	...
	public static class TaskManager {
		public static void runTaskWithProgress(ServerPlayer player, Runnable task, int totalSteps) {
			...
			}, executor).thenAccept(result -> player.sendSystemMessage(Component.literal("Task completed!"))).exceptionally(throwable -> {
	...
	public static class AsyncCommand {
		public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
			dispatcher.register(literal("asynctask").executes(AsyncCommand::executeAsyncTask));
		}
