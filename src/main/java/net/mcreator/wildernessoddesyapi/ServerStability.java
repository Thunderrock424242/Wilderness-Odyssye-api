...
public class ServerStabilityMod {
    ...
    private static final Map<ServerPlayer, Integer> playerTickCounts = new HashMap<>();
    private static final Map<ServerLevel, Integer> worldTickCounts = new HashMap<>();
    ...
    private void performServerStartTickTasks() {
        ...
        System.out.println("Server start tick tasks are being performed.");
    }
    ...
    private void performServerEndTickTasks() {
        ...
        System.out.println("Server end tick tasks are being performed.");
    }
    ...
    public static class EventHandler {
        ...
        public void onWorldTick(WorldTickEvent event) {
            if (!event.world.isClientSide) {
                ...
                performWorldTickTasks((ServerLevel) event.world);
            }
        }
        ...
        private void performPlayerTickTasks(ServerPlayer player) {
            ...
            playerTickCounts.put(player, playerTickCounts.getOrDefault(player, 0) + 1);
            if (playerTickCounts.get(player) % 100 == 0) {
                System.out.println("Player " + player.getName().getString() + " has been active for " + playerTickCounts.get(player) + " ticks.");
            }
        }

        private void performWorldTickTasks(ServerLevel world) {
            // Example logic for tasks to run on each world tick
            // This could include managing world events, updating entities, etc.
            worldTickCounts.put(world, worldTickCounts.getOrDefault(world, 0) + 1);
            if (worldTickCounts.get(world) % 600 == 0) {
                System.out.println("World " + world.dimension().location() + " has been active for " + worldTickCounts.get(world) + " ticks.");
            }
        }
        ...
        private void initializeServerResources() {
            ...
            System.out.println("Initializing server resources...");
        }
